<template>
  <view class="u-tabs" :style="{
    ...
    <scroll-view scroll-x :scroll="isScroll" class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation
      :style="{ zIndex: zIndex + 1 }">
      <view style="flex-direction: column;">
        <view style="flex-direction: row;">
          <view v-for="(item, index) in getTabs" :key="index" :id="'a' + index" class="align-center justify-center"
            style="flex-wrap: nowrap;padding-left: 15rpx;padding-right: 15rpx;height: 84rpx;" :style="[tabItemStyle(index)]"
            @tap="emit(index)">
            
            <text class="font-md text-dark-3 family-default" :class="current==index ? 'font-weight-bold' : ''">{{ item[name] || item['name']}}</text>
          </view>
        </view>
      ...
      tabItemStyle() {
        return (index) => {
          let style = {
            ...
            color: this.tabsInfo.length > 0 ? (this.tabsInfo[index] ? this.tabsInfo[index].color : this.activeColor) :
              this.inactiveColor,
        ...
        return new Promise((resolve, reject) => {
          setTimeout(() => {
            ...
            for (let i = 0; i < this.list.length; i++) {
              view.select('#a' + i).boundingClientRect();
            }
            view.exec(res => {
              const arr = [];
